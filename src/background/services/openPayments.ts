      ...
      authenticatedRequestInterceptor: async (request) => {
        ...
        const headers = await this.createHeaders({
          request: {
            method: request.method,
            url: request.url,
            headers: JSON.parse(JSON.stringify(Object.fromEntries(request.headers))),
        ...
        if (body) {
          ...
          originalRequest.headers.set('Content-Digest', headers['Content-Digest']!)
        }

        originalRequest.headers.set('Signature', headers['Signature']);
        originalRequest.headers.set('Signature-Input', headers['Signature-Input']);
